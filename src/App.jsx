...
import { BookOpen, Calendar as CalendarIcon, MapPin, Save, Moon, Sun, CheckSquare, ArrowUp, History, ChevronRight, Clock } from 'lucide-react';

const App = () => {
  // --- State Management with Lazy Initialization (修復重新整理後紀錄消失的問題) ---
  
  const [activeTab, setActiveTab] = useState('read'); // 'read', 'calendar', 'history'
  
  const [bookmarks, setBookmarks] = useState(() => {
    try {
      const saved = localStorage.getItem('ksitigarbha_bookmarks_v4');
      if (saved) return JSON.parse(saved);
      // Fallback migration for v3
      const savedV3 = localStorage.getItem('ksitigarbha_bookmark_v3');
      return savedV3 ? [JSON.parse(savedV3)] : [];
    } catch (e) { return []; }
  });

  const [calendarHistory, setCalendarHistory] = useState(() => {
    try {
      const saved = localStorage.getItem('ksitigarbha_calendar_v3');
  ...
  const [darkMode, setDarkMode] = useState(() => {
    try {
      const saved = localStorage.getItem('ksitigarbha_theme');
  ...
  // Load Data only on mount, but save changes immediately (Lazy Init handles initial state)
  // No loading state needed for data integrity anymore

  // Save Data
  useEffect(() => {
    localStorage.setItem('ksitigarbha_bookmarks_v4', JSON.stringify(bookmarks));
  }, [bookmarks]);

  useEffect(() => {
    localStorage.setItem('ksitigarbha_calendar_v3', JSON.stringify(calendarHistory));
  }, [calendarHistory]);

  useEffect(() => {
    localStorage.setItem('ksitigarbha_theme', JSON.stringify(darkMode));
  }, [darkMode]);
  ...
  // Handle Text Click (Add to Bookmarks)
  const handleSentenceClick = (volume, chapterIdx, sentence, index) => {
    const now = new Date();
    const newBookmark = {
      ...
      previewText: sentence.substring(0, 10) + '...'
    };
    
    // Add new bookmark to front, keep only top 5
    setBookmarks(prev => {
      const updated = [newBookmark, ...prev].slice(0, 5);
      return updated;
    });
    
    setToastMsg(`紀錄已儲存 (${now.getHours()}:${String(now.getMinutes()).padStart(2,'0')})`);
    setShowToast(true);
    setTimeout(() => setShowToast(false), 2000);
  };

  // Resume Progress (Accepts specific bookmark or defaults to latest)
  const handleResume = (targetBookmark) => {
    const bm = targetBookmark || bookmarks[0];
    if (bm) {
      setActiveTab('read');
      
      // Delay to ensure rendering if switching tabs
      setTimeout(() => {
        const key = `${bm.volume}-${bm.chapterIndex}-${bm.sentenceIndex}`;
        if (sentenceRefs.current[key]) {
          sentenceRefs.current[key].scrollIntoView({ behavior: 'smooth', block: 'center' });
          // Highlight effect
          sentenceRefs.current[key].classList.add('ring-2', 'ring-offset-2', 'ring-red-500');
          setTimeout(() => {
             sentenceRefs.current[key]?.classList.remove('ring-2', 'ring-offset-2', 'ring-red-500');
          }, 2000);
        }
      }, 100);
    }
  ...
  // Jump to Volume
  const scrollToVolume = (vol) => {
    if (volumeRefs.current[vol]) {
      volumeRefs.current[vol].scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  };

  // Calendar Logic
  const toggleVolumeComplete = (dateStr, volKey) => { // volKey: 'v1', 'v2', 'v3'
    setCalendarHistory(prev => {
      const currentDay = prev[dateStr] || { v1: false, v2: false, v3: false };
      return {
        ...prev,
        [dateStr]: {
          ...currentDay,
          [volKey]: !currentDay[volKey]
        }
      };
    });
  };
    ...
    <div 
      ...
      className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all active:scale-95 select-none
        ${checked 
          ...
          : (darkMode ? 'bg-stone-800 border-stone-700 text-stone-400' : 'bg-white border-amber-200 text-stone-600')}`}
    >
      <div className={`w-6 h-6 rounded-md flex items-center justify-center border-2 
        ${checked ? 'border-white bg-white/20' : 'border-current'}`}>
  ...
  return (
    <div className={`fixed inset-0 flex flex-col font-sans transition-colors duration-300 overflow-hidden select-none ${ThemeColors}`}>
      
      {/* Header with Safe Area Top */}
      <header className={`shrink-0 pt-[env(safe-area-inset-top)] shadow-md z-10 
        ${darkMode ? 'bg-stone-800' : 'bg-amber-100'}`}>
        <div className="flex justify-between items-center p-4">
          <div className="flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-amber-600" />
            <h1 className="text-lg font-bold">地藏經隨身記</h1>
          </div>
          <div className="flex gap-2">
            {bookmarks.length > 0 && (
               <button 
               onClick={() => handleResume(bookmarks[0])}
               className="text-xs bg-amber-600 text-white px-3 py-1.5 rounded-full flex items-center gap-1 shadow hover:bg-amber-700 active:scale-95 transition-transform"
             >
               <MapPin size={12} /> 續讀
             </button>
            )}
            <button 
              ...
              className={`p-2 rounded-full ${darkMode ? 'bg-stone-700 text-yellow-400' : 'bg-amber-200 text-amber-900'}`}
            >
      ...
      <main className="flex-1 overflow-hidden relative flex flex-col w-full">
        ...
        {activeTab === 'read' && (
          <div className="flex-1 flex flex-col h-full relative">
            ...
            {/* Volume Jump Links (Sticky) */}
            <div className={`flex overflow-x-auto p-2 border-b shrink-0 z-10 gap-2 no-scrollbar
              ${darkMode ? 'border-stone-700 bg-stone-900/95' : 'border-amber-200 bg-amber-50/95'}`}>
              {volumeKeys.map((vol) => (
                <button
                  key={vol}
                  onClick={() => scrollToVolume(vol)}
                  className={`px-4 py-2 rounded-full text-sm font-bold transition-all border whitespace-nowrap active:scale-95
                    ${darkMode 
                      ? 'border-stone-600 hover:bg-stone-700' 
                      : 'border-amber-200 bg-white hover:bg-amber-100 text-amber-800'}`}
                >
                  {vol}
                </button>
              ))}
            ...
            <div 
              ...
              className="flex-1 overflow-y-auto pb-24 scroll-smooth overscroll-contain"
            >
              <div className="max-w-xl mx-auto p-4 space-y-12">
                 
                 {/* Render ALL Volumes and Chapters */}
                 {volumeKeys.map((vol) => (
                   <div key={vol} ref={el => volumeRefs.current[vol] = el} className="scroll-mt-20">
                     {/* Volume Header Divider */}
                     <div className="flex items-center gap-4 my-8 opacity-50">
                       <div className="h-px bg-current flex-1"></div>
                       <span className="font-bold text-lg">{vol}</span>
                       <div className="h-px bg-current flex-1"></div>
                     </div>

                     {/* Chapters in this Volume */}
                     {sutraData[vol].map((chapter, chapterIdx) => (
                       <div key={chapter.title} className="mb-10">
                         <h2 className={`text-center text-2xl font-bold mb-6 mt-2 ${vol === '迴向' || vol === '前言' ? 'text-stone-500' : 'text-amber-600'}`}>
                           {chapter.title}
                         </h2>
                         
                         <div className={`text-xl leading-relaxed tracking-wide text-justify ${darkMode ? 'text-stone-300' : 'text-stone-800'}`}>
                           {splitTextToSentences(chapter.content).map((sentence, sIdx) => {
                             // Check if this sentence matches the LATEST bookmark
                             const isLatestBookmark = bookmarks.length > 0 && 
                                                  bookmarks[0].volume === vol && 
                                                  bookmarks[0].chapterIndex === chapterIdx && 
                                                  bookmarks[0].sentenceIndex === sIdx;
                             
                             const refKey = `${vol}-${chapterIdx}-${sIdx}`;

                             return (
                               <span
                                 key={sIdx}
                                 ref={el => sentenceRefs.current[refKey] = el}
                                 onClick={() => handleSentenceClick(vol, chapterIdx, sentence, sIdx)}
                                 className={`cursor-pointer transition-colors duration-200 rounded px-1 py-0.5 active:bg-amber-200/50
                                   ${isLatestBookmark 
                                      ? (darkMode ? 'bg-amber-900 text-amber-200' : 'bg-amber-200 text-amber-900 font-bold') 
                                      : ''
                                   }
                                 `}
                               >
            ...
            {showScrollTop && (
              <button 
                ...
                className="absolute bottom-6 right-6 p-3 rounded-full bg-amber-600 text-white shadow-lg shadow-amber-600/30 active:scale-95 transition-all z-20"
              >
                <ArrowUp size={24} />
              </button>
            )}
        ...
        {/* --- CALENDAR TAB --- */}
        {activeTab === 'calendar' && (
          <div className="flex-1 flex flex-col h-full animate-in fade-in zoom-in duration-300 overflow-hidden">
             {/* Calendar Header & Grid (Scrollable part) - Added extra bottom padding (pb-64) so content isn't hidden by the absolute panel */}
             <div className="flex-1 overflow-y-auto p-6 pb-64 overscroll-contain">
                <div className="max-w-md mx-auto">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setViewDate(new Date(viewDate.setMonth(viewDate.getMonth() - 1)))} className="p-1 hover:bg-black/10 rounded"><ArrowUp className="-rotate-90" size={20}/></button>
                        <span className="font-mono text-lg font-bold w-24 text-center">
                    ...
                    <div className="bg-white/5 rounded-2xl p-4 shadow-inner border border-white/10 mb-8">
                      {/* Days Header */}
                      <div className="grid grid-cols-7 mb-4 text-center text-sm opacity-60">
                      ...
                      <div className="grid grid-cols-7 gap-2">
                        ...
                        {Array.from({ length: getDaysInMonth(viewDate).daysInMonth }).map((_, i) => {
                          ...
                          const data = calendarHistory[dateStr] || { v1: false, v2: false, v3: false };
                          
                          const isSelected = getTodayStr(selectedDate) === dateStr;
                          const isToday = getTodayStr(new Date()) === dateStr;

                          return (
                            <div 
                              key={day}
                              onClick={() => setSelectedDate(dateObj)}
                              className={`aspect-square rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all active:scale-90 border-2 relative
                                ${isSelected
                                  ? 'border-amber-500 bg-amber-500/10'
                                  : (darkMode ? 'border-stone-700 bg-stone-800' : 'border-amber-100 bg-white')}
                                ${isToday && !isSelected ? 'ring-2 ring-blue-400 ring-offset-1 dark:ring-offset-stone-900' : ''}
                              `}
                            >
                              <span className={`font-bold ${isSelected ? 'text-amber-600' : 'opacity-70'}`}>{day}</span>
                              
                              {/* Dots Indicators */}
                              <div className="flex gap-0.5 mt-1">
                                {[data.v1, data.v2, data.v3].map((d, idx) => (
                                  <div key={idx} className={`w-1.5 h-1.5 rounded-full ${d ? 'bg-amber-500' : 'bg-gray-300 dark:bg-stone-600'}`} />
                                ))}
                              </div>
                            </div>
                          );
             ...
             {/* Bottom Panel: Checkboxes for Selected Date */}
             <div className={`p-6 rounded-t-3xl shadow-[0_-5px_20px_-5px_rgba(0,0,0,0.1)] z-20 absolute bottom-0 w-full pb-[calc(1.5rem+env(safe-area-inset-bottom))]
                ${darkMode ? 'bg-stone-800 border-t border-stone-700' : 'bg-white border-t border-amber-100'}`}>
                
                <div className="flex justify-between items-end mb-4 max-w-md mx-auto">
                   <div>
                     <div className="text-xs opacity-50 uppercase font-bold tracking-wider">編輯進度</div>
                     <div className="text-xl font-bold font-mono text-amber-600">
                       {selectedDate.getFullYear()}/{selectedDate.getMonth()+1}/{selectedDate.getDate()}
                     </div>
                   </div>
                   {getTodayStr(selectedDate) === getTodayStr(new Date()) && (
                     <span className="text-xs bg-amber-100 text-amber-800 px-2 py-1 rounded dark:bg-amber-900 dark:text-amber-100">今天</span>
                   )}
                </div>

                <div className="grid grid-cols-3 gap-3 max-w-md mx-auto">
                   {['v1', 'v2', 'v3'].map((key, idx) => {
                     const dateStr = getTodayStr(selectedDate);
                     const checked = calendarHistory[dateStr]?.[key] || false;
                     return (
                       <Checkbox 
                         key={key}
                         label={checkboxKeys[idx]}
                         checked={checked}
                         onChange={() => toggleVolumeComplete(dateStr, key)}
                       />
                     );
                   })}
                </div>
        ...
        {/* --- HISTORY TAB --- */}
        {activeTab === 'history' && (
          <div className="flex-1 overflow-y-auto p-6 animate-in fade-in zoom-in duration-300 overscroll-contain">
             <div className="max-w-md mx-auto">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-bold flex items-center gap-2">
                    <History /> 閱讀紀錄
                ...
                <div className="space-y-3">
                  {bookmarks.length === 0 ? (
                     <div className="p-8 rounded-xl text-center opacity-50 bg-black/5 dark:bg-white/5">
                       <History size={48} className="mx-auto mb-4 opacity-50" />
                       <p>尚無紀錄</p>
                       <p className="text-xs mt-2">請至經文頁面點擊文句以新增標記</p>
                     </div>
                  ) : (
                    bookmarks.map((bm, idx) => (
                      <div 
                        key={bm.id || idx} 
                        className={`p-4 rounded-xl flex items-center justify-between border group transition-all active:scale-98
                           ${darkMode ? 'bg-stone-800 border-stone-700 hover:border-amber-500/50' : 'bg-white border-amber-100 hover:border-amber-300 shadow-sm'}
                        `}
                      >
                         <div className="flex-1 min-w-0 pr-4">
                            <div className="flex items-center gap-2 text-xs opacity-60 mb-1.5 uppercase tracking-wide">
                               <Clock size={12} /> {formatTime(bm.timestamp)}
                               <span className="w-1 h-1 rounded-full bg-amber-500" />
                               {bm.volume}
                            </div>
                            <div className={`font-medium text-lg leading-snug line-clamp-2 ${darkMode ? 'text-stone-300' : 'text-stone-700'}`}>
                               {bm.previewText}
                            </div>
                         </div>
                         ...
                         <button 
                           ...
                           className="p-3 rounded-full bg-amber-100 text-amber-800 dark:bg-stone-700 dark:text-amber-400 opacity-80 hover:opacity-100 hover:bg-amber-200 dark:hover:bg-stone-600 transition-all shadow-sm"
                         >
      ...
      <nav className={`shrink-0 border-t flex justify-around p-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] 
         ${darkMode ? 'bg-stone-800 border-stone-700' : 'bg-white border-amber-200'}`}>
        
        <button 
          onClick={() => setActiveTab('read')}
          className={`flex flex-col items-center gap-1 p-2 rounded-xl w-20 transition-all active:scale-90
            ${activeTab === 'read' 
              ? 'text-amber-600 bg-amber-50 dark:bg-stone-700 dark:text-amber-400' 
              : 'opacity-50'}`}
        >
          <BookOpen size={24} />
          <span className="text-[10px] font-bold">閱讀經文</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold">功課表</span>
        </button>
        ...
        >
          <History size={24} />
          <span className="text-[10px] font-bold">閱讀紀錄</span>
        </button>

      </nav>

      {/* Toast */}
      <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-6 py-4 rounded-xl bg-black/80 backdrop-blur text-white text-center shadow-2xl transition-all duration-200 z-50
        ${showToast ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
        <Save className="mx-auto mb-2 text-amber-400" size={32} />
        <div className="font-bold text-lg">進度已儲存</div>
        <div className="text-xs opacity-80 mt-1 text-stone-300">{toastMsg}</div>
      </div>
